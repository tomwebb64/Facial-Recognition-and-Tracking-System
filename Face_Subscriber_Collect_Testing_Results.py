...
def startNode():
    ...
    name_pub = rospy.Publisher('names', String, queue_size=1)
    rate = rospy.Rate(50) 
...
def process_image(img_ros):
    global close_preview
    global name_pub
    global test_itterations
    global test_positive
    ...
    if test_itterations == 100:
        if continue_test == "y":
            test_type = raw_input("What test paramiters were used for these itterations?")
            # Open the file in append & read mode ('a+')
            with open("test_results.txt", "a+") as file_object:
                ...
                file_object.write(test_type+": "+str(test_positive)+"/100")
                continue_test = raw_input("Would you like to run another test? (y/n)")
                if continue_test == "y":
                    test_itterations = 0
                    test_positive = 0
        else:
